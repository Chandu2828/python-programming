...
- upper() - Changes all the characters in the string to uppercase
- lower() - Changes all the characters in the string to lowercase
...
- splitlines() - Split the given string based on new line character
...
- Similarly, if the end index is not included, python will print it until the end of the string
- we can use step size to fetch elements at a specified interval i.e., `[start:end:step]`
  - If the step value is not specified, then python considers its value to be 1
  - For e.g. to reverse a string we use: `[::-1]`

# Properties

- String are immutable i.e. the elements that are once defined can't be altered
...
- pop() _ To pop off an item from the list at a given index and is an inplace operation
- reverse() - Reverse a given list
- sort() - Sort given a list and is an inplace operation
- sorted() - Sort and return the sorted list 
